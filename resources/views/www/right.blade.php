<div class="bg-white mb-4 p-3">
    ...
    <div class="tab-content clearfix wan-video">
        <div class="tab-pane fade show active" id="vp">
            @isset($film)
                <div class="mb-4 position-relative">
                    <a href="{{ route('video.show', $film) }}" title="{{ $film->title }}" target="_blank">
            ...
            <a class="row text-right" href="{{ route('video.list') }}" target="_blank">
        ...
        <div class="tab-pane fade" id="zb">
            @isset($live)
                <div class="mb-4 position-relative">
                    <a href="{{ route('video.show', $live) }}" title="{{ $live->title }}" target="_blank">
...
@isset($province_rel)
    ...
    @if(count($tops['activities']))
        <div class="bg-white mb-4 p-3">
            ...
            @endforeach
            <a class="row text-right" href="{{ route('activity.list', ['pid' => $province_rel->id]) }}" target="_blank">
...
@else
    <div class="bg-white mb-4 p-3">
        ...
        @foreach($raiders as $raider)
            <a href="{{ route('raider.show', $raider) }}" title="{{ $raider->title }}" target="_blank">
        ...
        @endforeach
        <a class="row text-right" href="{{ route('raider.list') }}" target="_blank">
...
<div class="bg-white p-3">
    ...
    @foreach($travels as $travel)
        <a href="{{ route('travel.show', $travel) }}" title="{{ $travel->title }}" target="_blank">
    ...
    <a class="row text-right" href="{{ route('travel.list') }}" target="_blank">
